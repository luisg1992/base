...
use Modules\Persona\Http\Resources\TipoDocumentoIdentidadCollection;

trait TipoDocumentoIdentidadDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipos de Doc Identidad';
        $this->titulo_tabla = 'TIPOS DE DOC IDENTIDAD';
        $this->nombre_tabla = 'TipoDocIdentidad';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('Descripcion')->label('Tipo de Documento')->sortable(true));
        $builder->agregarColumna(Column::make('Abreviatura')->label('Abreviatura')->sortable(true));
        $builder->agregarColumna(Column::make('CodigoSUNASA')->label('Codigo Sunasa')->sortable(true));
        $builder->agregarColumna(Column::make('CodigoHIS')->label('Codigo HIS ')->sortable(true));
        $builder->agregarColumna(Column::make('CodigoSIS')->label('Codigo SIS')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query = TipoDocumentoIdentidad::query()
        ...
        $query->orderBy('IdDocIdentidad', 'asc');
        return (new TipoDocumentoIdentidadCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
