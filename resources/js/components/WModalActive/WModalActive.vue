...
async function openDialog(recordId) {
    ...
    loading.value = true;
    try {
        const { data } = await axios.get(`/${props.resource}/record_active/${recordId}`)
...
async function onSubmit() {
    ...
    try {
        const { data } = await axios.post(`/${props.resource}/change_active`, form.value)
        if (data.success) {
...
<template>
    <Dialog v-model:visible="isDialogOpen"
            ...
            :closable="false"
            :style="{ width: '480px' }">
        <div class="b-dialog-active">
            <div class="left">
                <div class="icon">
                    <i class="ti ti-shield-off" style="font-size: 36px;" />
                </div>
            </div>
            <div class="right">
                <div class="title">Cambiar estado del registro</div>
                <div class="description">
                    ¿Seguro que desea cambiar el estado del registro?.
                </div>
                <div v-if="form.verify_password" class="verify-password">
                    <label class="verify-password-label" for="password">Por favor ingresar su contraseña</label>
                    <Password
                        id="password"
