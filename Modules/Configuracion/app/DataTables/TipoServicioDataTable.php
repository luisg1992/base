...
use Modules\Configuracion\Http\Resources\TipoServicioCollection;

trait TipoServicioDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipos de Servicios';
        $this->titulo_tabla = 'TIPOS DE SERVICIOS';
        $this->nombre_tabla = 'TipoServicio';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('Descripcion')->label('Tipo de Servicio')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query->orderBy('IdTipoServicio');
        return (new TipoServicioCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
