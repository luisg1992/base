...
use Modules\Configuracion\Http\Resources\CitaAnuladaMotivoCollection;
//use Modules\Configuracion\Resources\CitaAnuladaMotivoCollection;
use Modules\Configuracion\Models\CitaAnuladaMotivo;

trait CitaAnuladaMotivoDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Citas Anuladas Motivos';
        $this->titulo_tabla = 'CITAS ANULADAS MOTIVOS';
        $this->nombre_tabla = 'CitaAnuladaMotivo';
    ...
    public function records(Request $request)
    {
        ...
        $query->orderBy('IdMotivo');
        return (new CitaAnuladaMotivoCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
