...
use Modules\Configuracion\Http\Resources\RecetaFrecuenciaCollection;
use Modules\Configuracion\Models\RecetaFrecuencia;

trait RecetaFrecuenciaDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Receta Frecuencia';
        $this->titulo_tabla = 'RECETA FRECUENCIAS';
        $this->nombre_tabla = 'RecetaFrecuencia';
    ...
    public function records(Request $request)
    {
        ...
        $query->orderBy('IdRecetaFrecuencia');
        return (new RecetaFrecuenciaCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
