...
trait RecetaViaAdministracionDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Receta Via Administracion';
        $this->titulo_tabla = 'RECETA VIA ADMINISTRACION';
        $this->nombre_tabla = 'RecetaViaAdministracion';
    ...
    public function records(Request $request)
    {
        ...
        $query = RecetaViaAdministracion::query()
        ...
        $query->orderBy('IdViaAdministracion');
        return (new RecetaViaAdministracionCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
