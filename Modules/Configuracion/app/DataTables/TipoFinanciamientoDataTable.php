...
use Modules\Configuracion\Http\Resources\TipoFinanciamientoCollection;
use Modules\Configuracion\Models\TipoFinanciamiento;

trait TipoFinanciamientoDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipo Financiamiento';
        $this->titulo_tabla = 'TIPO FINANCIAMIENTO';
        $this->nombre_tabla = 'TipoFinanciamiento';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('TipoComprobante')->label('Tipo Comprobante')->sortable(true));
        $builder->agregarColumna(Column::make('TipoConcepto')->label('Tipo Concepto')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query->orderBy('IdTipoFinanciamiento');
        return (new TipoFinanciamientoCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
