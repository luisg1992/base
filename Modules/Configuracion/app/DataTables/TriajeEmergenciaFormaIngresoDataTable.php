...
use Modules\Configuracion\Http\Resources\TriajeEmergenciaFormaIngresoCollection;

trait TriajeEmergenciaFormaIngresoDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Formas de Ingreso';
        $this->titulo_tabla = 'FORMAS DE INGRESO';
        $this->nombre_tabla = 'TriajeEmergenciaFormasIngresos';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('Descripcion')->label('Forma de Ingreso')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query = TriajeEmergenciaFormaIngreso::query()
        ...
        $query->orderBy('IdFormaIngreso');
        return (new TriajeEmergenciaFormaIngresoCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
