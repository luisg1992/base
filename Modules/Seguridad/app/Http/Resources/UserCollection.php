...
class UserCollection extends ResourceCollection
{
    ...
    public function toArray(Request $request): Collection
    {
        ...
        return $this->collection->transform(function ($row, $key) use ($valorPermission, $acciones) {
            ...
            $name = [
                'Nro. Doc.: '.$row->empleado->DNI,
                $row->name,
            ];

            return [
                'id' => $row->id,
                'name' => Cell::multiLine($name),
                'email' => $row->email,
                'contacto' => Cell::multiLine([$row->Celular, $row->CorreoElectronico]),
                'empleado_nombre' => $row->empleado->ApellidoPaterno . ' ' . $row->empleado->ApellidoMaterno . ', ' . $row->empleado->Nombres,
