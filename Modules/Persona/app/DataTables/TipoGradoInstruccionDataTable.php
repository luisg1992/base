...
use Modules\Persona\Http\Resources\TipoGradoInstruccionCollection;

trait TipoGradoInstruccionDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipo Grado Instruccion';
        $this->titulo_tabla = 'TIPOS GRADO INSTRUCCION';
        $this->nombre_tabla = 'TipoGradoInstruccion';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('Descripcion')->label('Tipo Grado Instruccion')->sortable(true));
        $builder->agregarColumna(Column::make('sip2000')->label('sip2000')->sortable(true));
        $builder->agregarColumna(Column::make('codigoReniec')->label('Codigo Reniec')->sortable(true));
        $builder->agregarColumna(Column::make('idTipoDIRIS')->label('Tipo Diris')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query = TipoGradoInstruccion::query()
        ...
        $query->orderBy('IdGradoInstruccion', 'asc');
        return (new TipoGradoInstruccionCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
