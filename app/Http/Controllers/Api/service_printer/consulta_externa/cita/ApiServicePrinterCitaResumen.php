...
class ApiServicePrinterCitaResumen extends Controller
{
    public function service_printer_cita_resumen(Request $request)
    {
        ...
        // Verificamos si existe el campo 'Grupo' 
        $Grupo = null;
        $Formato = 'ticket';
        $Usuario = '';
        if (isset($valoresArray['Grupo']) && isset($valoresArray['Formato']) && isset($valoresArray['Usuario'])) {
            $Grupo = $valoresArray['Grupo'];
        ...
        $codigo_Cita_Resumen_Ticket = 'ERROR';
        $nombreArchivo_Cita_Resumen_Ticket ='CITA_RESUMEN.pdf';

        $grupo = isset($valoresArray['Grupo']) && is_array($valoresArray['Grupo'])
            ? implode(',', $valoresArray['Grupo'])
            : ($valoresArray['Grupo'] ?? '');
        ...
        $response = $citasController->CitasBuscarResumenFormatoPDF(new Request($Parametros));
        $responseJson = $response->getContent();
        $responseData = json_decode($responseJson, true);

        if ($responseData['success']) {
            $codigo_Cita_Resumen_Ticket = 'OK';
            $Cita_Resumen_Ticket = array(
                'codigo' => $codigo_Cita_Resumen_Ticket,
                'ImpresionPorDefecto' => $Ticket[0]->PorDefecto,
                'NombreImpresora' => $Ticket[0]->Impresora,
                'Formato' => $Ticket[0]->Formato,
                'nombre_pdf' => $nombreArchivo_Cita_Resumen_Ticket,
                'type' => 'pdf',
            ...
            return response()->json([
                'respuesta' => true,
                'data' => [
                    'Cita_Resumen_Ticket' => $Cita_Resumen_Ticket
                ]
            ], 202);
        }
