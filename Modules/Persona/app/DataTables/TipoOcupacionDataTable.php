...
use Modules\Persona\Http\Resources\TipoOcupacionCollection;

trait TipoOcupacionDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipo Ocupacion';
        $this->titulo_tabla = 'TIPOS Ocupacion';
        $this->nombre_tabla = 'TipoOcupacion';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('descripcion')->label('Tipo Ocupacion')->sortable(true));
        $builder->agregarColumna(Column::make('lolcli')->label('lolcli')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query->orderBy('IdTipoOcupacion', 'asc');
        return (new TipoOcupacionCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
