...
const handleOpen = async () => {
    ...
    if (props.recordId) {
        title.value = props.viewRecord ? 'Visualizar admisión emergencia' : 'Editar admisión emergencia';
        try {
            accionForm.value = 'visualizar';
            let {data} = await axios.get(`/${props.resource}/record/${props.recordId}`)
            Object.assign(form.value, data.data)
            await obtenerInformacionTriaje(data.data.IdTriajeEmergencia);
            showOptions();
        } catch (error) {
            if (error.status === 403) {
                closeDialog();
        ...
        title.value = 'Registrar admisión emergencia';
        if (props.triajeId) {
            accionForm.value = 'registrar_con_triaje';
            await obtenerInformacionTriaje(props.triajeId);
            let formTriajeTemp = JSON.parse(JSON.stringify(formTriaje.value));
            form.value.IdPaciente = formTriajeTemp.IdPaciente;
        } else {
            accionForm.value = 'registrar';
        }
    }
}
...
const validarCuentasPendientes = async () => {
    const response = await axios.get(`/${props.resource}/buscar-cuentas-pendientes/${form.value.IdPaciente}`);
    let data = response.data;
    if (data.success) {
        tieneCuentasPendientes.value = false;
        tieneCuentasPendientesMensaje.value = null;
    } else {
        tieneCuentasPendientes.value = true;
        tieneCuentasPendientesMensaje.value = data.mensaje;
    }
}
...
const successPaciente = async (datos) => {
    ...
    formTriaje.value.IdTipoDocTriaje = datos.IdDocIdentidad;
    formTriaje.value.NroDocTriaje = datos.NroDocumento;
    formTriaje.value.ApellidoPaternoTriaje = datos.ApellidoPaterno;
    formTriaje.value.ApellidoMaternoTriaje = datos.ApellidoMaterno;
    formTriaje.value.PrimerNombreTriaje = datos.PrimerNombre;
    formTriaje.value.SegundoNombreTriaje = datos.SegundoNombre ?? null;
    formTriaje.value.TercerNombreTriaje = datos.TercerNombre ?? null;
    formTriaje.value.FechaNacimientoTriaje = datos.FechaNacimiento;
    formTriaje.value.IdFuenteFinanciamiento = datos.IdFuenteFinanciamiento;
    const now = new Date();
    ...
    if (confirmado) {
        ...
        if (success) {
            try {
                ...
                formAdmisionTemp.IdPaciente = formTriajeTemp.IdPaciente;
                formAdmisionTemp.IdTipoGravedad = formTriajeTemp.IdTipoGravedad;
                formAdmisionTemp.IdTipoFinanciamiento = formTriajeTemp.IdTipoFinanciamiento;
                formAdmisionTemp.IdFuenteFinanciamiento = formTriajeTemp.IdFuenteFinanciamiento;
                formAdmisionTemp.IdServicioIngreso = formTriajeTemp.IdServicio;

                formAdmisionTemp.IdTriajeEmergencia = IdTriajeEmergencia;
...
<template>
    <BaseModal :isVisible="isDialogOpen"
        ...
        <div class="row">
            ...
            <div class="col-12" v-if="tieneCuentasPendientes">
                <div class="alert alert-danger mt-3 mb-0 text-center">{{ tieneCuentasPendientesMensaje }}</div>
            </div>
            <template v-if="!tieneCuentasPendientes && form && (form.IdPaciente !== null || idPaciente !== null)">
                <div class="col-12">
                    <Tabs value="0">
                        <TabList>
