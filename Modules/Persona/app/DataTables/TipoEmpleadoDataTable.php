...
use Modules\Persona\Http\Resources\TipoEmpleadoCollection;

trait TipoEmpleadoDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipos Empleados';
        $this->titulo_tabla = 'TIPOS EMPLEADOS';
        $this->nombre_tabla = 'TipoEmpleado';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('Descripcion')->label('Tipo de Empleado')->sortable(true));
        $builder->agregarColumna(Column::make('TipoEmpleadoHIS')->label('Tipo Empleado HIS')->sortable(true));
        $builder->agregarColumna(Column::make('EsProgramado')->label('Programado')->sortable(true));
        $builder->agregarColumna(Column::make('TipoEmpleadoSIS')->label('Tipo Empleado SIS  ')->sortable(true));
        $builder->agregarColumna(Column::make('EsColegiatura')->label('Colegiatura')->sortable(true));
        $builder->agregarColumna(Column::make('TipoEspecialidadSIS')->label('Tipo Especialidad SIS')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query->orderBy('IdTipoEmpleado', 'asc');
        return (new TipoEmpleadoCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
