<script setup>
import ModalFormulario from './FormularioRecetaDosisUnidadMedida.vue'
...
const resource = ref('configuracion/receta-dosis-unidad-medida')
const refTable = ref()
...
const successAction = () => {
    //appStore.fetchTabla('citaAnuladaMotivo');
    refTable.value.listarRegistros()
    cerrarModal()
}

const successActions = (data) => {
    console.log('successActions', data)
    if (data.action === 'view') {
        clickView(data.id)
    }
    if (data.action === 'edit') {
