...
use Modules\Configuracion\Http\Resources\TipoGravedadAtencionCollection;
use Modules\Configuracion\Models\TipoGravedadAtencion;

trait TipoGravedadAtencionDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | PRIORIDADES';
        $this->titulo_tabla = 'PRIORIDADES';
        $this->nombre_tabla = 'TiposGravedadAtencion';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('OrdenPrioridad')->label('Prioridad')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query = TipoGravedadAtencion::query()
        ...
        $query->orderBy('IdTipoGravedad');
        return (new TipoGravedadAtencionCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
