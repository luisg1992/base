...
use Modules\Persona\Http\Resources\TipoSexoCollection;

trait TipoProcedenciaDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipo Procedencia';
        $this->titulo_tabla = 'TIPOS PROCEDENCIA';
        $this->nombre_tabla = 'TipoProcedencia';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('Descripcion')->label('Tipo Procedencia ')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query->orderBy('IdProcedencia', 'asc');
        return (new TipoProcedenciaCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
