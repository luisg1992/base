...
use Modules\Configuracion\Http\Resources\TipoConsultorioCollection;

trait TipoConsultorioDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipos de Consultorios';
        $this->titulo_tabla = 'TIPOS DE CONSULTORIOS';
        $this->nombre_tabla = 'TipoConsultorio';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('Descripcion')->label('Tipo de Consultorio')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query->orderBy('IdTipoConsultorio');
        return (new TipoConsultorioCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
