...
use Modules\Configuracion\Http\Resources\TipoMotivoTriajeEmergenciaCollection;
use Modules\Configuracion\Models\TipoMotivoTriajeEmergencia;

trait TipoMotivoTriajeEmergenciaDataTable
{
    use PaginationTrait;

    public function inicializarTabla()
    {
        $this->titulo_pagina = config('app.name') . ' | Tipo Motivo Triaje Emergencia';
        $this->titulo_tabla = 'TIPO MOTIVO TRIAJE EMERGENCIA ';
        $this->nombre_tabla = 'Tipo Motivo Triaje Emergencia';
    ...
    private function obtenerColumnasTabla(): array
    {
        ...
        $builder->agregarColumna(Column::make('TipoGravedadAtencion')->label('Prioridad')->sortable(true));
    ...
    public function records(Request $request)
    {
        ...
        $query = TipoMotivoTriajeEmergencia::query()
            ->with(['prioridad', 'servicio'])
        ...
        return (new TipoMotivoTriajeEmergenciaCollection($query->paginate($this->limit)))->additional($this->metaAdditional);
    }
}
